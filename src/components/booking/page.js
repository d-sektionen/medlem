...
export default function NewBookingPage() {
  ...
  const myBookings = sortedBookings?.filter(
    (booking) =>
      booking.user.id === user.id && new Date(booking.start) >= new Date()
  )
  const unConfirmedBookings = sortedBookings?.filter(
  ...
  const resource = items?.find((item) => item.id === selectedResource)

  const isNarrow = useMediaQuery('(max-width: 600px)')

  const handleDelete = (bookingId) => {
    openConfirmation('Är du säker på att du vill ta bort bokningen?', () =>
      destroy(bookingId)
    )
  }
    ...
    <div className={page}>
      ...
      <div className={content}>
        <div className={resourceSelector}>
          <BookableResourceContainer
            items={sortedItemsWithIcons}
            selectedItem={selectedResource}
            onSelectedItemChange={handleSelectedResourceChange}
          />
          {isNarrow && (
        ...
        <div className={calendarPadding}>
          <BookingCalendar
            bookings={bookings}
            showBookingCount={resource?.count > 1}
          />
        </div>
        ...
        <div className={bookingList}>
          ...
          <div className={infoBox}>
            <p>
              Genom att boka{' '}
              {items?.find((item) => item.id === selectedResource)?.name}{' '}
              godkänner du{' '}
              <a
                href={
                  items?.find((item) => item.id === selectedResource)?.terms
                }
              >
                bokningsavtalet
              </a>
          ...
          <CreateNewBooking
            selectedItemId={selectedResource}
            items={items}

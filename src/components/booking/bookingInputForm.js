...
export const BookingInputForm = ({
  ...
  const [startTime, setStartTime] = useState(
    booking?.start ? new Date(booking?.start)?.toTimeString().split(' ')[0] : ''
  )
  const [endDate, setEndDate] = useState(
    booking?.end ? new Date(booking?.end)?.toISOString().split('T')[0] : ''
  )
  const [endTime, setEndTime] = useState(
    booking?.end ? new Date(booking?.end)?.toTimeString().split(' ')[0] : ''
  )
  const [errors, setErrors] = useState({})
  const [description, setDescription] = useState(booking?.description)
  const [count, setCount] = useState(booking?.count ?? 1)
      ...
      <form onSubmit={onSubmit} className={form}>
        <div className={formBlock}>
          ...
          <div className={formRow}>
            ...
            <input
              ...
              required={true}
              className={errors.startTime ? errorInput : ''}
            ></input>
          </div>
          {errors.startDate && (
            <div className={errorMessage}>{errors.startDate}</div>
          )}
        </div>
        <div className={formBlock}>
          ...
          <div className={formRow}>
            <input
              ...
              required={true}
              className={errors.endDate ? errorInput : ''}
            ></input>
            <input
              name="endTime"
              id="endTime"
              ...
              required={true}
              className={errors.endTime ? errorInput : ''}
            ></input>
          </div>
          {errors.endDate && (
            <div className={errorMessage}>{errors.endDate}</div>
          )}
        </div>
        {errors.duration && (
        ...
        <div>
          ...
          <textarea
            ...
            placeholder="Skriv ditt ändamål här"
          ></textarea>
          {errors.description && (
